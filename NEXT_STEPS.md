...
a hundred two-qubit gates. Four classes have now been checked against those
criteria.

Quantum ML (Result 67) is the interesting one and the only class that *fits* the
hardware — six two-qubit gates per kernel entry, accuracy under device noise
identical to exact simulation. It also genuinely beats classical kernels, 0.911
against 0.829 on 11 of 12 seeds. But only on data generated by its own feature
map; on two Gaussian blobs it sits at 0.528, chance. It fits because the circuit
is shallow, and it expresses little for the same reason.

**So the open question is now specific rather than general:** does any
naturally-occurring dataset have the structure a shallow quantum feature map
matches? That is answerable with real data and a few hundred lines, and nothing
here has tried it. It is the one direction left where a positive result is
possible without waiting for 800× better hardware.
