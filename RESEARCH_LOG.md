...
### Result 67 — quantum machine learning: the one class that fits the hardware, and the one where the problem is defined by the algorithm

NEXT_STEPS said three problem classes had been ruled out with measurements and
that nothing had looked beyond them. That was not quite honest: quantum machine
learning was named in the original task and **this project dismissed it by
argument** — "the classical baseline is too strong" — without measuring it. That
is precisely the move Results 42, 50 and 64 each punished, twice after the
quantum machinery had already been built.

So it was measured, on the same three criteria (`qres/problems/learning.py`).

**Criterion 1, required accuracy: passes trivially.** Classification tolerates
percent-level error. This is what makes QML superficially the most promising of
the four.

**Criterion 3, encoding cost: passes — the only class that does.** A ZZ
feature-map fidelity kernel on two features needs **6 two-qubit gates per kernel
entry** after transpilation. Against Result 61's budget that is comfortable, and
it shows: accuracy under a `heron` noise model is **identical to exact
statevector**. Nothing else in this project has survived contact with a device
unchanged.

**Criterion 2, the classical baseline.** Kernel ridge classifiers over a grid of
linear, RBF and polynomial kernels with a ridge sweep, 12 seeds, 60 test points
each, paired on identical splits:

| dataset | **classical** | **quantum** | quantum wins |
|---|---|---|---|
| linear (two Gaussian blobs) | 0.971 ± 0.022 | **0.528 ± 0.075** | **0/12** |
| moons (interleaving crescents) | 0.983 ± 0.014 | **0.693 ± 0.064** | **0/12** |
| ad hoc (Havlíček construction) | 0.829 ± 0.131 | **0.911 ± 0.133** | **11/12** |

**On linearly separable data the quantum kernel is at 0.528 — chance.** On moons
it is 0.693 against an RBF kernel's 0.983, losing every seed on both.

**On `ad_hoc` it genuinely wins**: 0.911 against 0.829, on 11 of 12 seeds. That
is a real, replicated, 8-point advantage and it should be stated as such.

And `ad_hoc` is *generated by the feature map the kernel is built from* — a
random diagonal observable is measured on the feature-mapped state and its sign
is the label. The kernel is matched to the data by construction. This is the
dataset behind the field's central positive result, included here so the
circularity is measured rather than argued about.

**So the four classes complete like this:**

| | accuracy requirement | classical baseline | encoding cost |
|---|---|---|---|
| chemistry | **fails** (0.019%) | **fails** (FCI, 100 ms) | — |
| MaxCut | passes | **fails** (1 ms hill-climb) | passes |
| HP folding | passes | **fails** (3/4 optima in <1 min) | **fails** (1 538 gates at N=6) |
| **quantum ML** | passes | **passes, on its own data only** | **passes** |

**QML is the only class that fits the hardware, and it fits because the circuits
are shallow — which is the same reason the kernel expresses so little.** Six
two-qubit gates is a similarity measure with almost no structure in it, and on
data that did not come from that measure it is worse than a straight line.

That is a different failure from the other three and worth separating: chemistry,
MaxCut and folding fail on *cost* or on *a strong competitor*. QML's advantage is
real and reproducible but exists only where **the problem was defined by the
algorithm**. A method that beats classical kernels by 8 points on its own
generated data and sits at chance on two Gaussian blobs has demonstrated a
matched filter, not a learner. Whether any naturally-occurring dataset has the
structure this kernel matches is the open question, and nothing here answers it.

**A bug that was flattering the wrong side, caught by a test rather than a
benchmark.** The first version of `ad_hoc` thresholded the observable at zero. A
random ±1 diagonal has no reason to have zero expectation over feature-mapped
states, and it did not: **94% of one class on two of three seeds, 10% on the
third.** On a dataset where a constant classifier scores 0.94, the original
numbers (classical 0.968, quantum 1.000) were measuring almost nothing.

Thresholding at the median instead gives 0.40–0.57 positive across seeds — and
the corrected comparison is *more* favourable to the quantum kernel, not less:
the imbalance had been inflating the classical baseline with majority-class
prediction and hiding a genuine 8-point gap. The bug flattered the side this
project has spent sixty-six results defending, which is worth noting: the
discipline is supposed to cut both ways, and here it did.

**One efficiency, since the same lesson has now appeared four times.** The first
kernel implementation transpiled every entry separately — identical circuit
structure, only parameters differing — and made one simulator call per entry. On
a 40-sample dataset that is 1 120 transpilations and 1 120 calls, and it had not
finished after 25 minutes. Transpiling the parameterised template once and
running in batches of 256: **18 seconds**, at least 80×. The identical mistake
and fix appear in Results 28, 52 and 56. The expensive thing is never the
computation; it is the bookkeeping around it.

### Result 68 — the quantum kernel matches no structural family, only its own output

Result 67 left the QML line with one specific question, and it was the only one
in this project where a positive result was still possible without waiting for
800× better hardware: the quantum kernel beats classical kernels by 8 points on
`ad_hoc`, which its own feature map generated. **Is that pure circularity, or
does it match a structural family that some real dataset might also have?**

The ZZ feature map is periodic in every feature with period 2π and its
entangling layer encodes pairwise differences. So three datasets were built with
exactly that structure but *not* generated by the map — plus a classical
**periodic kernel** added to the baseline grid, so periodic data would not hand
the quantum side a win an RBF simply cannot express:

| dataset | **classical** | **quantum** | quantum wins |
|---|---|---|---|
| linear | 0.973 ± 0.021 | 0.532 ± 0.077 | **0/10** |
| moons | 0.983 ± 0.013 | 0.692 ± 0.059 | **0/10** |
| **periodic** — `sign(cos(x₀+x₁))` | 0.973 ± 0.013 | **0.522 ± 0.065** | **0/10** |
| **interaction** — `sign(cos(x₀−x₁))` | 0.967 ± 0.020 | **0.545 ± 0.037** | **0/10** |
| **parity** — 4 features | 0.685 ± 0.024 | **0.512 ± 0.047** | **0/10** |
| ad hoc — generated by the map | 0.840 ± 0.143 | **0.902 ± 0.144** | **9/10** |

**It is at chance on every structure it should match.** Periodic data: 0.522.
Data built from precisely the pairwise-difference form its entangling layer
encodes: 0.545. It wins nowhere except on its own output.

The `parity` row is the sharpest. That is the one dataset where the *classical*
side is weak — 0.685, the only value under 0.96 outside `ad_hoc` — so there is
real room for a different similarity measure to help. The quantum kernel scores
**0.512**. Where there is room, it does not fill it.

**So the answer is circularity, and the QML line closes.** The kernel is a matched
filter for one specific generative process — the one it is built from. It is not
a feature map that captures periodicity, interactions, or parity in any usable
sense; it captures the exact distribution of `|⟨φ(x)|φ(y)⟩|²` and nothing else.
The question "does any naturally-occurring dataset have this structure" now has
a much narrower meaning than when it was posed: the structure is not *periodic
data* or *interacting data*, it is *data whose labels were produced by measuring
this circuit*.

**Which completes all four problem classes, every criterion measured:**
...
| MaxCut | passes | **fails** (1 ms hill-climb) | passes |
| HP folding | passes | **fails** (3/4 optima in <1 min) | **fails** (1 538 gates at N=6) |
| quantum ML | passes | **fails** (chance on every non-self-generated structure) | passes |

Four classes, four failures, four different reasons — cost, a strong competitor,
